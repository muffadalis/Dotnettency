    ...
    public class TenantShellFactory : ITenantShellFactory<Tenant>
    {
        public Task<TenantShell<Tenant>> Get(TenantIdentifier distinguisher)
        {
            if (distinguisher.Uri.Port == 5004)
            {
                ...
                var tenant = new Tenant(tenantId) { Name = "Moogle", ConnectionString = "Filename=./piranha.moogle.db" };
            ...
            if (distinguisher.Uri.Port == 5000 || distinguisher.Uri.Port == 5001 || distinguisher.Uri.Port == 44354)
            {
                ...
                var tenant = new Tenant(tenantId) { Name = "Gicrosoft", ConnectionString = "Filename=./piranha.gicrosoft.db" };
                var result = new TenantShell<Tenant>(tenant, new Uri("https://localhost:5000"), new Uri("https://localhost:5001"), new Uri("http://localhost:44354")); // additional distinguishers to map this same tenant shell instance too.
                return Task.FromResult(result);
            }
